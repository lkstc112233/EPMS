 ...
 * 导入免费师范生数据
 */
public class TableOperation extends AnnualAction{
	...
	public TableOperation() throws SQLException, NoSuchFieldException, SecurityException, InstantiationException, IllegalAccessException{
		super();
...
//	public Base getUpdateBase(){return this.updateBase;}	public void setUpdateBase(Base b){this.updateBase=b;}
		...
		if(clazz==null) this.search=null;
			...
			if(this.search==null|| !this.search.clazz.equals(clazz)){
				...
				try {
					...
					for(Search.Triple t:this.search.getRestraint()){
						if(t.field.getName().equals("year")){
							t.type=Search.RestraintType.Equal;
							t.value=String.valueOf(this.getYear());
						}
					}
	...
	public String execute(){//执行查询
		...
		if(usr.isSchoolUser()){
			for(Search.Triple t:this.search.getRestraint()){
				if(!Search.Triple.isFitSchool(t.field,t.value,usr.getSchool())){
					session.put(token.ActionInterceptor.ErrorTipsName,
							"必须筛选当前学院（"+usr.getSchool()+"）对应的学院和专业！");
					return NONE;
				}
			}
		}
	...
	public String update(){
		...
		if(usr.isSchoolUser()){
			boolean ok=true;
			try{
				Field f=Base.getField(b.getClass(),"school");
				Object o=f.get(b);
				String v=o==null?null:o.toString();
				if(!Search.Triple.isFitSchool(f,v,usr.getSchool()))
					ok=false;
				if(ok){
					f=Base.getField(b.getClass(),"major");
					o=f.get(b);
			...
			if(!ok){
				session.put(token.ActionInterceptor.ErrorTipsName,
						"只能修改当前学院（"+usr.getSchool()+"）对应的学院和专业！");
				return NONE;
			}
		}
		//===
...
//			b.update(updateBase);
	...
	public String create(){
		...
		if(usr.isSchoolUser()){
			boolean ok=true;
			try{
				Field f=Base.getField(this.createNewBase.getClass(),"school");
				f.setAccessible(true);
				Object o=f.get(this.createNewBase);
				String v=o==null?null:o.toString();
				if(!Search.Triple.isFitSchool(f,v,usr.getSchool()))
					ok=false;
			}catch(Exception e){}
			try{
				if(ok){
					Field f=Base.getField(this.createNewBase.getClass(),"major");
					f.setAccessible(true);
					Object o=f.get(this.createNewBase);
					String v=o==null?null:o.toString();
					if(!Search.Triple.isFitSchool(f,v,usr.getSchool()))
